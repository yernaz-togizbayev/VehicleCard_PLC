...
public class Bus extends Vehicle {
	...
	public Bus(int id, String brand, String model, int yearBuilt, double basePrice, int lastInspection) {
	...
	public String getType() {
		return Bus.class.getSimpleName();
	}

	@Override
	double getDiscount() {
		double totalDiscount = 0.03 * (2020 - getYearBuilt()) * getBasePrice();
		
		if (lastInspection > getYearBuilt())
			totalDiscount += 0.03 * (2020 - lastInspection) * getBasePrice();
		
		if (totalDiscount > 0.25 * getBasePrice())
			return 0.25 * getBasePrice();
